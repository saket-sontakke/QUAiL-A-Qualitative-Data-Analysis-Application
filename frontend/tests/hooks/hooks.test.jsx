...
//     // });
//     await act(async () => {
//       await result.current.handleSelectFile(mockProject.importedFiles[1]);
//     });
...
//     expect(result.current.confirmModalData.message).toContain('Failed to import text file.');
//   });
...
//     const projectWithNewCode = { ...mockProject, codeDefinitions: [...mockProject.codeDefinitions, { _id: 'new-def', name: 'New Code' }] };
//     mockAxios.onPost(`${import.meta.env.VITE_BACKEND_URL}/api/projects/test-project-id/code-definitions`).reply(200, { project: projectWithNewCode });
...
 * @description This file contains a comprehensive set of unit and integration tests
 * for the `useProjectViewHooks`, covering initialization, state management, API interactions,
 * and user-facing functionality, now testing with the AuthContext.
 */
...
import { vi, describe, it, expect, beforeEach, afterEach } from 'vitest';
import useProjectViewHooks from '../../src/pages/hooks/Hooks.jsx';
import { AuthProvider } from '../../src/pages/auth/AuthContext.jsx'; // 1. Import AuthProvider
...
// 2. REMOVED the entire mockLocalStorage object. It's no longer needed.

// Mock browser APIs to simulate a realistic environment for the hook.
const mockRange = {
    setStart: vi.fn(),
    setEnd: vi.fn(),
    getClientRects: vi.fn(() => [{ bottom: 100, right: 200, left: 100, top: 80, width: 100, height: 20 }]),
    commonAncestorContainer: document.createElement('div')
};
...
Object.defineProperty(window, 'getSelection', { value: vi.fn(() => mockSelection), writable: true });
Object.defineProperty(document, 'createRange', { value: vi.fn(() => mockRange), writable: true });
Object.defineProperty(document, 'createTreeWalker', { value: vi.fn(() => ({ nextNode: vi.fn(() => null) })), writable: true });
...
/**
 * 3. A new, comprehensive wrapper that provides both a mocked React Router
 * and our AuthContext to the hook during tests.
 */
...
    name: 'Test Project',
    importedFiles: [
      { _id: 'file-1', name: 'test-file-1.txt', content: 'This is test content for file 1. It has multiple sentences. Each sentence can be coded or highlighted.' },
      { _id: 'file-2', name: 'test-file-2.txt', content: 'This is another test file content.' }
    ],
    codedSegments: [
      { _id: 'segment-1', fileId: 'file-1', text: 'test content', startIndex: 8, endIndex: 20, codeDefinition: { _id: 'code-def-1', name: 'Important', color: '#FF0000' } }
    ],
    inlineHighlights: [
      { _id: 'highlight-1', fileId: 'file-1', text: 'highlighted', startIndex: 68, endIndex: 79, color: '#FFFF00' }
    ],
    codeDefinitions: [
      { _id: 'code-def-1', name: 'Important', description: 'Important content', color: '#FF0000' }
    ],
    memos: [
      { _id: 'memo-1', fileId: 'file-1', title: 'Test Memo', content: 'This is a test memo', text: 'memo text', startIndex: 50, endIndex: 59, author: 'test-user', authorId: 'user-1' }
    ]
};
...
    vi.clearAllMocks();
    ...
    // 4. Mock the user data that our AuthContext will provide by mocking localStorage.getItem('user')
    const mockUser = {
        _id: 'mock-user-id',
        name: 'mock-username',
        token: 'mock-token'
    ...
    vi.spyOn(window.localStorage.__proto__, 'getItem').mockImplementation(key => {
        ...
        return null; // for other localStorage calls like theme or UI preferences
    });

    mockSelection.toString.mockReturnValue('');
    mockSelection.rangeCount = 0;
    mockSelection.anchorNode = null;
    mockRange.getClientRects.mockReturnValue([{ bottom: 100, right: 200, left: 100, top: 80, width: 100, height: 20 }]);
});
...
    it('should initialize with correct default values', async () => {
        mockAxios.onGet(`${import.meta.env.VITE_BACKEND_URL}/api/projects/test-project-id`).reply(200, mockProject);
        const { result } = renderHook(() => useProjectViewHooks(), { wrapper: AuthWrapper });
        expect(result.current.project).toBe(null);
        expect(result.current.projectName).toBe('');
        expect(result.current.loading).toBe(true);
        await waitFor(() => { expect(result.current.loading).toBe(false); });
...
    it('should handle fetch project error', async () => {
        mockAxios.onGet(`${import.meta.env.VITE_BACKEND_URL}/api/projects/test-project-id`).reply(500, { message: 'Server error' });
        const { result } = renderHook(() => useProjectViewHooks(), { wrapper: AuthWrapper });
        await waitFor(() => { expect(result.current.loading).toBe(false); });
        expect(result.current.error).toBe('Failed to load project');
    });
...
        const { result } = renderHook(() => useProjectViewHooks(), { wrapper: AuthWrapper });
        await waitFor(() => expect(result.current.loading).toBe(false));
        await act(async () => { await result.current.handleSelectFile(mockProject.importedFiles[1]); });
        expect(result.current.selectedContent).toBe('This is another test file content.');
        expect(result.current.selectedFileName).toBe('test-file-2.txt');
    });

    it('should handle file upload successfully', async () => {
        const updatedProject = { ...mockProject, importedFiles: [...mockProject.importedFiles, { _id: 'file-3', name: 'new-file.txt', content: 'New file content' }] };
        mockAxios.onPost(`${import.meta.env.VITE_BACKEND_URL}/api/projects/import/test-project-id`).reply(200, { project: updatedProject });
        const { result } = renderHook(() => useProjectViewHooks(), { wrapper: AuthWrapper });
        await waitFor(() => expect(result.current.loading).toBe(false));
        const mockFile = new File(['test content'], 'new-file.txt', { type: 'text/plain' });
        const mockEvent = { target: { files: [mockFile] } };
        await act(async () => { await result.current.handleFileChange(mockEvent); });
        await waitFor(() => { expect(result.current.project.importedFiles).toHaveLength(3); });
    });
});

// ... ALL OTHER TEST SUITES (describe blocks) should also use `wrapper: AuthWrapper` for renderHook ...
// This change needs to be applied consistently throughout the file.

describe('useProjectViewHooks - AuthContext Integration', () => {
    it('should use user data from AuthContext in API calls', async () => {
        mockAxios.onGet(`${import.meta.env.VITE_BACKEND_URL}/api/projects/test-project-id`).reply(200, mockProject);
        mockAxios.onPost(`${import.meta.env.VITE_BACKEND_URL}/api/projects/test-project-id/memos`).reply(200, { project: mockProject });
        const { result } = renderHook(() => useProjectViewHooks(), { wrapper: AuthWrapper });
        await waitFor(() => expect(result.current.loading).toBe(false));
...
    it('should handle missing user data gracefully when context is empty', async () => {
        // Override the mock for this specific test to simulate a logged-out user
...
        const { result } = renderHook(() => useProjectViewHooks(), { wrapper: AuthWrapper });
        
        // The hook should immediately set an error because the token is missing
        await waitFor(() => {
          expect(result.current.error).toBe('Authentication error. Please log in.');
        });
    });
});